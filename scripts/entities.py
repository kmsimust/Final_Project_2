...
class Character(Entity):
    def __init__(self, pos, name, frames, frame_id,
        ...
        self.timers = {
            ...
            'notice': Timer(400, func=self.start_move)
        }
    ...
    def raycast(self):
        if check_connections(self.radius, self, self.player) and self.line_of_sight() and not self.moved and not self.noticed:
            self.player.block()
    ...
    def move(self, dt):
        if not self.moved and self.direction:
...
class Player(Entity):
    def __init__(self, pos, frames, groups, facing_direction, collision_sprites):
        ...
        self.hitbox = self.rect.inflate(-self.rect.width / 2, -80)
        self.noticed = False
